...
const ViewChart = ({ dashboardItemId, dashboardItemFilters }) => {
  ...
  console.log("ViewChart config:", { config, chartType, historicDataId });
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      if (!loading && chartType === "custom" && historicDataId) {
        ...
        try {
          const result = await engine.query(getAnalyticsQuery(historicDataId));
    ...
    fetchAnalytics();
  }, [loading, chartType, historicDataId, engine]);
  ...
  if (chartType === "custom") {
    ...
    return <CustomChart analyticsData={analyticsData} />;
  }
